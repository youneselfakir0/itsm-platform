  ...
  return (
    <div className="flex gap-6">
      <div className="flex-1">
        ...
        {showForm && (
          <form onSubmit={create} className="bg-slate-900 border border-slate-800 rounded-xl p-4 mb-4 space-y-2" aria-label={t('cmdb.form.title')}>
            <div className="space-y-1">
              <label htmlFor="ci-class" className="block text-xs text-slate-400">{t('cmdb.class')}</label>
            ...
            <div className="space-y-1">
              <label htmlFor="ci-name" className="block text-xs text-slate-400">{t('cmdb.name')}</label>
              <input id="ci-name" required className="w-full bg-slate-800 rounded-lg px-3 py-2" placeholder={t('cmdb.name')}
                value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
            </div>
            <div className="space-y-1">
              <label htmlFor="ci-attrs" className="block text-xs text-slate-400">{t('cmdb.attributes')}</label>
        ...
        <table className="w-full text-sm">
          <caption className="text-left text-xs text-slate-500 mb-1">{t('cmdb.caption')}</caption>
      ...
      {sel && (
        <div className="w-96 bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-3">
          ...
          <div>
            <p className="text-xs font-semibold text-slate-500 mb-1">{t('cmdb.relations')}</p>
