...
export default function Catalog({ user }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {items.map((i) => (
          <div key={i.id} onClick={() => { setSel(i); setFormData({}); }}
            className="bg-surface border border-surface hover:border-cyan-700 rounded-xl p-4 cursor-pointer">
      ...
      {sel && (
        <form onSubmit={submit} className="bg-surface border border-cyan-800 rounded-xl p-4 space-y-3 max-w-md">
          <h3 className="font-bold">{sel.name}</h3>
          {(sel.form_schema || []).map((f) => (
            <div key={f.name}>
              <label htmlFor={`cf-${f.name}`} className="text-sm text-muted">{f.label}</label>
              {f.type === 'select' ? (
                <select id={`cf-${f.name}`} className="w-full bg-input rounded-lg px-3 py-2"
                ...
                <input id={`cf-${f.name}`} required={f.required} type={f.type === 'number' ? 'number' : 'text'}
                  className="w-full bg-input rounded-lg px-3 py-2"
                  onChange={(e) => setFormData({ ...formData, [f.name]: e.target.value })} />
              )}
            </div>
          ))}
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => setSel(null)} className="bg-input px-4 py-2 rounded-lg text-sm">{t('catalog.cancel')}</button>
          </div>
        </form>
      )}
      <div>
        ...
        <div className="space-y-2">
          {requests.map((r) => (
            <div key={r.id} className="bg-surface border border-surface rounded-xl p-3 flex items-center gap-3">
              <span className="flex-1">{r.item_name}</span>
              <span className={`text-xs px-2 py-1 rounded ${r.status === 'approved' ? 'bg-green-900 text-green-300' : r.status === 'rejected' ? 'bg-red-900 text-red-300' : 'bg-input'}`}>{r.status}</span>
