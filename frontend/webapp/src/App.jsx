    ...
    <div className="flex items-center gap-1" role="group" aria-label={t('lang.select')}>
      {LANGS.map((l) => (
        <button key={l.code} type="button" onClick={() => setLang(l.code)}
          ...
          className={`text-xs px-2 py-1 rounded ${lang === l.code ? 'bg-cyan-600 text-white' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}>
          {l.label}
        </button>
      ))}
    </div>
    ...
    <div className="min-h-screen flex items-center justify-center">
      <form onSubmit={submit} className="bg-slate-900 p-8 rounded-2xl border border-slate-800 w-96 space-y-4" aria-labelledby="login-title">
        ...
        <div className="space-y-1">
          <label htmlFor="email" className="block text-sm text-slate-400">{t('login.email')}</label>
          <input id="email" name="email" type="email" autoComplete="username" required
            className="w-full bg-slate-800 rounded-lg px-3 py-2 outline-none focus:ring-2 ring-cyan-500"
            value={email} onChange={(e) => setEmail(e.target.value)} />
        </div>
        <div className="space-y-1">
          <label htmlFor="password" className="block text-sm text-slate-400">{t('login.password')}</label>
